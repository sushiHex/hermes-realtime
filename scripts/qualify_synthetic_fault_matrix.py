"""Qualify the synthetic fault matrix from a source-bound pure candidate wheel."""
...
from scripts.synthetic_fault_matrix import validate_synthetic_fault_v1
...
def main() -> None:
    ...
    receipt = SYNTHETIC_FAULT_REGISTRATION_V1.produce(archive, identity, wheel)
    evidence = validate_synthetic_fault_v1(receipt)
    print(
        canonical_json_bytes(
            {
                "schemaVersion": "synthetic-fault-matrix-summary-v1",
                "proofClass": "synthetic_injected",
                "caseCount": 5,
                **asdict(evidence),
            }
        ).decode("utf-8"),
